...
const FeaturedCard = ({
  ...
  >
    ...
    <p className="relative z-10 text-sm text-foreground/75 leading-relaxed mt-4 flex-1">
      {project.description}
    </p>
...
const CompactCard = ({
  ...
  >
    <div
      className={`absolute inset-0 bg-gradient-to-br ${project.accent} opacity-40 pointer-events-none`}
    />
  ...
  return (
    <section id="projects" className="section-container" ref={ref}>
      ...
      >
        ...
        <div className="w-24 h-0.5 bg-gradient-to-r from-foreground/50 to-transparent mb-4 mt-2" />
        <p className="section-subtitle">
          A selection of things I've built — from AI SaaS to blockchain DApps
      ...
      >
        {CATEGORIES.map((category) => (
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium border transition-colors ${active === category ? "bg-foreground text-background border-foreground" : "bg-transparent text-muted-foreground border-border hover:text-foreground hover:border-foreground/30"}`}
          >
            {category}
          </button>
        ))}
